...
class ConfigurationViewTest(TestCase):
    ...
    def setUpTestData(self):
        ...
        for model in range(self.number_of_models):
            model = MLModel.objects.create(
                name="Model %s" % model, code="Code %s" % model
            )
    ...
    def test_update(self):
        ...
        data = {
            ...
            "ml_models": [self.models[0].pk],
        }
        resp = self.client.put(
            "/configurations/" + str(self.configuration.pk),
            content_type="application/json",
            data=data,
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_create(self):
        ...
        resp = self.client.post(
            "/configurations/", content_type="application/json", data=data
        )
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
...
class ResultViewTest(TestCase):
    ...
    def test_upload_results(self):
        ...
        uploaded_file = SimpleUploadedFile(
            "test.h5", file.read(), content_type="multipart/form-data"
        )
        ...
        data = {"trained_model": uploaded_file, "data": json.dumps(results)}
        resp = self.client.post(
            "/results/" + str(self.resultID), data, format="multipart"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        filepath = (
        ...
        self.assertTrue(
            abs(float(self.obj.train_loss) - results["train_loss"]) <= epsilon
        )
